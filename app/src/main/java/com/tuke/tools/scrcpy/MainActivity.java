...
import com.tuke.tools.scrcpy.utils.PreUtils;
import com.tuke.tools.scrcpy.utils.Progress;
import com.tuke.tools.scrcpy.utils.ThreadUtils;
import com.tuke.tools.scrcpy.utils.Util;
...
public class MainActivity extends Activity implements Scrcpy.ServiceCallbacks, SensorEventListener {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
            ...
            if (first_time) {
                if (!Progress.isShowing()) {
                    isConnecting = true;
                    Progress.showDialog(MainActivity.this, getString(R.string.scrcpy_please_wait), "", false, MainActivity.this::cancelConnection);
                }
    ...
    public void scrcpy_main() {
        ...
        setContentView(R.layout.scrcpy_fragment_adb_helper);
...
//        });
    ...
    public void get_saved_preferences() {
        ...
        setSpinner(R.array.scrcpy_options_resolution_values, R.id.spinner_video_resolution, Constant.PREFERENCE_SPINNER_RESOLUTION);
        setSpinner(R.array.scrcpy_options_bitrate_keys, R.id.spinner_video_bitrate, Constant.PREFERENCE_SPINNER_BITRATE);
        setSpinner(R.array.scrcpy_options_delay_keys, R.id.delay_control_spinner, Constant.PREFERENCE_SPINNER_DELAY);
...
//            Log.d("fuck", "set_display_nd_touch_land: "+this_device_aspect_ratio);
    ...
    private void getAttributes() {
        ...
        final String[] videoResolutions = getResources().getStringArray(R.array.scrcpy_options_resolution_values)[videoResolutionSpinner.getSelectedItemPosition()].split("x");
        screenHeight = Integer.parseInt(videoResolutions[0]);
        screenWidth = Integer.parseInt(videoResolutions[1]);
        videoBitrate = getResources().getIntArray(R.array.scrcpy_options_bitrate_values)[videoBitrateSpinner.getSelectedItemPosition()];
        delayControl = getResources().getIntArray(R.array.scrcpy_options_delay_values)[delayControlSpinner.getSelectedItemPosition()];
    }
    ...
    private void start_screen_copy_magic() {
        ...
        isConnecting = false;
        Progress.closeDialog();
        setContentView(R.layout.scrcpy_surface);
        final View decorView = getWindow().getDecorView();
    ...
    public void errorDisconnect() {
        // 必须退出
        // 退出重连
        Dialog.displayDialog(this, getString(R.string.scrcpy_disconnect),
                getString(R.string.scrcpy_disconnect_ask), () -> {
                    if (serviceBound) {
                        showMainView();
                        first_time = true;
                    } else {
    ...
    private void cancelConnection() {
        isConnecting = false;
        Progress.closeDialog();
        Toast.makeText(context, R.string.scrcpy_connection_cancelled, Toast.LENGTH_SHORT).show();
        // 如果已经绑定了服务，需要断开
        if (serviceBound) {
            showMainView(true);
            first_time = true;
    ...
    private void connectScrcpyServer(String serverAdr) {
        if (!TextUtils.isEmpty(serverAdr)) {
            ...
            isConnecting = true;
            Progress.showDialog(MainActivity.this, getString(R.string.scrcpy_please_wait), "", false, this::cancelConnection);
            ThreadUtils.workPost(() -> {
                // 检查是否已被取消
                if (!isConnecting) {
                    return;
    ...
    protected void connectExitExt(boolean userDisconnect) {
        ...
        if (headlessMode && !resumeScrcpy && !result_of_Rotation) {
            ...
            if (!userDisconnect) {
                Dialog.displayDialog(this, getString(R.string.scrcpy_connect_faild),
                        getString(R.string.scrcpy_connect_faild_ask), () -> {
                            // 重试连接
                            connectScrcpyServer(PreUtils.get(context, Constant.CONTROL_REMOTE_ADDR, ""));
                        }, () -> {
